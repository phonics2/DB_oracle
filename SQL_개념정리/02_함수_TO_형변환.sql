...
SELECT 1234, TO_CHAR(1234,'0000') FROM DUAL; -- 지정된 자리값보다 변환해야하는 값의 길이가 더 크기 때문에 ### 출력

-- 화폐기호 + 자릿수 구분
SELECT 
      TO_CHAR(123456789,'L999999999'), -- L 9 아홉 개
      TO_CHAR(123456789,'$999999999'), -- $ 9 아홉 개
      TO_CHAR(123456789,'$999,999,999') -- $ 9 아홉 개
FROM DUAL;
-- L의 경우 현재 시스템에서 사용하는 화폐 단위 기호 한국 = 

-- 모든 사원의 연봉 조회 현재 화폐 단위 , 사용해서 조회
...
FROM DUAL;
-- DUAL 테이블 사용해서 현재 날짜(SYSDATE) 2024-06-05 (수) 오전 시:MI:초
SELECT TO_CHAR(SYSDATE,'YYYY-MM-DD (DY) AM HH:MI:SS')
FROM DUAL;
-- DUAL 테이블 사용해서 현재 날짜(SYSDATE) 2024-06-05 오전 00시 00분 00초 현재 시
SELECT TO_CHAR(SYSDATE,'YYYY"년" MM"월" DD"일" AM HH"시" MI"분" SS"초" "현재"HH"시"') AS "시간"
FROM DUAL;
...
-- 50 이상 : Y R 현재 세기(2000 년대 기준 이후) R 이전 세기(1900년대 기준)
-- 50 미만 : Y,R 둘다 누락된 연도 앞부분에 현재 2000년대 세기 추가
-- 연도가 앞 두자리 누락 50년대 기준으로 살펴봄

-- 1) 50년대 미만 확인
SELECT 
      TO_DATE('49-11-01','YY-MM-DD') AS "YY",  -- 2049
      TO_DATE('49-11-01','RR-MM-DD') AS "RR"   -- 2049
FROM DUAL;      
