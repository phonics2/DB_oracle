...
- 관계형 데이터베이스 SQL 이용해 테이블간 관계를 맺는 방법

- 관계형 데이터 베이스는 최소한의 데이터를 테이블에 담고 있어
    원하는 정보를 테이블에서 조회하려면 한 개 이상의 테이블에서
     데이터를 읽어와야 하는 경우가 많음
     이 때, 테이블간 관계를 맺기 위한 연결고리 역할이 필요한데
     두 테이블에서 같은 데이터를 저장하는 컬러밍 견결고리가 됨.
...
-- 1) LEFT [OUTER] JOIN
-- 합치기 사용한 두 테이블 중에서 왼쪽 편에 작성된 테이블의
-- 컬럼수를 기준으로 JOIN
...
-- 모든 행이 최종 결과(RESULT SET)에 포함되도록 하는 조인
...
-- 왼쪽에 있는 컬럼수에 맞게 표시를 넣어라(NULL 값이라도 넣어라)




-- 2) RIGHT [OUTER] JOIN
-- 합치기 사용한 두 테이블 중에서 오른쪽 편에 작성된 테이블의
-- 컬럼수를 기준으로 JOIN
...
WHERE DEPT_CODE(+) = DEPT_ID;
-- DEPT_CODE와 DEPT_ID 값이 일치하지 않아도
-- 오른쪽에 있는 컬럼수에 맞게 표시를 넣어라(NULL 값이라도 넣어라)




-- 2) FULL [OUTER] JOIN
-- 합치기 사용한 두 테이블 이 가진 모든 행을 결과에 포함
