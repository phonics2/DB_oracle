...
FROM EMPLOYEE
WHERE DEPT_CODE NOT LIKE 'D5';          -- WHERE DEPR_CODE !='D5';   가능
...
FROM EMPLOYEE
WHERE JOB_CODE NOT LIKE 'J3';

--29. `BONUS`가 0.1이 아닌 사원의 이름과 보너스
SELECT EMP_NAME , BONUS
FROM EMPLOYEE
WHERE BONUS NOT LIKE 0.1;

--30. `MANAGER_ID`가 200이 아닌 사원의 이름과 매니저 ID.
---- 숫자의 경우 '' 사용하지 않아도 사용가능
SELECT EMP_NAME , MANAGER_ID
FROM EMPLOYEE
WHERE MANAGER_ID NOT LIKE 200;
...
/*
-- 42. SECLET EMP_NAME, HIRE_DATE
FROM EMPLOYEE
WHERE HIRE_DATE BETWEEN('2000-01-01','YYYY-MM-DD')
AND TO_DATE ('2010-12-31','YYYY-MM-DD');        가능
*/
...
SELECT EMP_NAME , HIRE_DATE
FROM EMPLOYEE
WHERE TO_CHAR(HIRE_DATE,'YYYYMMDD') NOT BETWEEN 20000101 AND 20101231;
