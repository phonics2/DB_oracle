...
-- 2. 학과의 학과 정원을 조회
SELECT DEPARTMENT_NAME || '의 정원은 ' || CAPACITY || '명 입니다.' AS"결과"
FROM TB_DEPARTMENT;
...
-- 4. 도서관에서 대출 도서 장기 연체자들을 찾아 이름 게시하고자 한다.
----- 그 대상자들의 학번이 다음과 같을 때 대상자를 찾는 적절할 SQL 쿼리 구문
...
-- 춘 기술대학교 총장의 이름을 알아낼수 있는 SQL 문장 작성 총장은 NULL
SELECT PROFESSOR_NAME
FROM TB_PROFESSOR
WHERE DEPARTMENT_NO IS NULL;

--7.
-- 수강신청을 하려고 한다. 선수과목 여부를 확인하는데,
-- 선수과목이 존재하는 과목들은 어떤 과목인지 과목번호 조회
SELECT * FROM TB_CLASS; --  조회하기

SELECT CLASS_NO
FROM TB_CLASS
WHERE PREATTENDING_CLASS_NO IS NOT NULL;

-- 8. DISTINCT 활용해서       -- DISTINCT 는 중복 제거 구문
...
-- 10. 춘 대학교 교수중에 이름이 세 글자가 아닌 교수가 두명 있음
-- 그 교수의 이름과 주민번호 조회
--  WHERE LIKE    FROM TB_PROFESSOR
SELECT * FROM TB_PROFESSOR;

SELECT PROFESSOR_NAME AS "이름" , PROFESSOR_SSN AS "주민번호"
FROM TB_PROFESSOR
WHERE PROFESSOR_NAME NOT LIKE '___';  -- 글자 몇글자인지 ___ 3글자
...
FROM TB_GRADE TB_STUDEN
WHERE STUDENT_NO = 'A517178';

--> 소수점 제거 소수점 몇 자리 까지 표기
---> 소수점 1자리만까지 표기
----> ROUND(AVG(POINT),1)

-- 17. 지도교수를 배정받지 못한 학생의 수 조회
-- TB_STUDENT 에서 COACH_PRO
SELECT COUNT(*)
FROM TB_STUDENT
