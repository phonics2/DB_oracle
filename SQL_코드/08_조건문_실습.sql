...
-- 1) 주어진 학생의 성별 기준으로 성별 출력
 SELECT STUDENT_NAME,STUDENT_SSN, DECODE(SUBSTR(STUDENT_SSN,8,1),'1','남자','2','여자','기타') AS "성별"
 FROM TB_STUDENT;
 

-- 2) SELECT EMP_NAME, DEPT_CODE FROM EMPLOYEE
SELECT EMP_NAME, DEPT_CODE,DECODE(DEPT_CODE,'D1','인사관리부','D2','회계관리부','D3','마케팅부','D4','국내영업부','기타') AS "부서이름"
FROM EMPLOYEE;
/*
D1 인사관리부
D2 회계관리부
...
SELECT EMP_NAME, JOB_CODE, DECODE(JOB_CODE,'J1','대표','J2','부사장','J3','부장','J4','차장','J5','과장','J6','대리','J7','사원','알수 없음') AS "직급"
FROM EMPLOYEE;
/*
J1 대표
J2 부사장
...
-- 1) 급여에 따라 등급 설정 600만이상 S1 500만이상 S2 400만이상 S3 나머지 S4 급여등급 표기
