...
--> 서브 쿼리를 통해서 특정 조건에 만족하는 최저 급여를 찾고
----> 조건에 맞는 급여를 받는 직원에 외부 쿼리에서 조회

/*
서브쿼리
SELECT MIN(SALARY) --> 컬럼 선택
FROM EMPLOYEE      --> 찾을 데이터 테이블 선택
WHERE SALARY >= 3000000; --> 급여가 300만 이상인 행들만 최소값을 계산

외부쿼리 = 메인쿼리
SELECT EMP_NAME, SALARY  --> 최종으로 눈으로 볼 컬럼 선택
FROM EMPLOYEE            --> 테이블 선택
WHERE SALARY = (서브쿼리);--> 급여가 서브퀴리에서 반환된 최소 급여와 같은 행들만 가져오는 것
*/
...
SELECT DEPT_TITLE,
    (SELECT COUNT(*)
    FROM EMPLOYEE E
    WHERE E.DEPT_CODE = D.DEPT_ID) AS "부서별 인원 수"
FROM DEPARTMENT D;
