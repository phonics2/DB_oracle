...
SELECT * FROM TB_STUDENT;
SELECT STUDENT_NO AS "학번", STUDENT_NAME AS"학생이름" ,STUDENT_ADDRESS AS "거주지 주소"
FROM TB_STUDENT
...
-- ANSI
SELECT P.PROFESSOR_NAME, P.PROFESSOR_SSN  --컬럼명이 같을때는 어디가 어딘지 구분 해줘야함
FROM TB_PROFESSOR P
JOIN TB_DEPARTMENT D ON(P.DEPARTMENT_NO = D.DEPARTMENT_NO) AND D.DEPARTMENT_NO = '005'
...
SELECT STUDENT_NO AS "학번", TO_CHAR(POINT,'FM9.00') AS "학점"
FROM TB_GRADE
...
-- ANSI
SELECT C.CLASS_NAME AS "과목이름" , D.DEPARTMENT_NAME AS "학과"
FROM TB_CLASS C
...
-- ANSI
SELECT S.STUDENT_NO AS "학번", S.STUDENT_NAME AS "학생이름", ROUND(SUM(G.POINT),1) AS "평점"
FROM TB_STUDENT S
JOIN TB_DEPARTMENT D ON(S.DEPARTMENT_NO = D.DEPARTMENT_NO) AND S.DEPARTMENT_NO = '059'
JOIN TB_GRADE G ON(S.STUDENT_NO = G.STUDENT_NO)
GROUP BY S.STUDENT_NO, S.STUDENT_NAME;
-- ORACLE
SELECT S.STUDENT_NO AS "학번", S.STUDENT_NAME AS "학생이름", ROUND((G.POINT),1) AS "평점"
FROM TB_STUDENT S, TB_DEPARTMENT D, TB_GRADE G
WHERE S.DEPARTMENT_NO = D.DEPARTMENT_NO
AND S.DEPARMENT_NO IN('059')
