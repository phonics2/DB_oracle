...
SELECT EMP_NAME, EMP_ID, EMAIL, LENGTH(EMAIL) AS "이메일 길이"
FROM EMPLOYEE;
...
FROM EMPLOYEE
WHERE  SUBSTR(PHONE,-4) IS NOT NULL;

-- 3. 주민등록번호의 생년월일 추출
--> EMP_NAME EMP_NO   AS 생년월일
SELECT EMP_NO, SUBSTR(EMP_NO,1,6) AS "생년월일"
FROM EMPLOYEE;
...
/******REPLACE 활용 실습*********/
-- 1. 이메일 주소에서 모든 (.) 하이픈(-) 대체
--> EMP_ID, EMAIL, AS 수정된이메일
