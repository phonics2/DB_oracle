...
SELECT JOB_CODE, AVG(SALARY) AS "평 균"
FROM EMPLOYEE
GROUP BY JOB_CODE;

3. EMPLOYEE 테이블에서 SAL_LEVEL별 보너스 합계를 조회
SELECT SAL_LEVEL, SUM(BONUS) AS "보너스 합"
FROM EMPLOYEE
GROUP BY SAL_LEVEL;
...
SELECT EMP_NAME
FROM EMPLOYEE
WHERE HIRE_DATE > TO_DATE('2000-01-01','YYYY-DD-MM');
...
SELECT EMP_NAME
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5' -- IN ==    IN('D5','D6) 을 사용해도 됨
OR DEPT_CODE = 'D6';
